...
module.exports = class {
  ...
  deployRelease(done) {
    ...
    async.series([this.connectToRemoteTask.bind(this), this.onBeforeDeployTask.bind(this), this.onBeforeDeployExecuteTask.bind(this), this.compressReleaseTask.bind(this), this.createReleaseFolderOnRemoteTask.bind(this), this.uploadArchiveTask.bind(this), this.uploadReleaseTask.bind(this), this.decompressArchiveOnRemoteTask.bind(this), this.updateSharedSymbolicLinkOnRemoteTask.bind(this), this.createFolderTask.bind(this), this.makeDirectoriesWritableTask.bind(this), this.makeFilesExecutableTask.bind(this), this.onBeforeLinkTask.bind(this), this.onBeforeLinkExecuteTask.bind(this), this.updateCurrentSymbolicLinkOnRemoteTask.bind(this), this.onAfterDeployTask.bind(this), this.onAfterDeployExecuteTask.bind(this), this.remoteCleanupTask.bind(this), this.deleteLocalArchiveTask.bind(this), this.closeConnectionTask.bind(this)], function (err, result) {
  ...
  removeRelease(done) {
    ...
    async.series([this.connectToRemoteTask.bind(this), this.removeReleaseTask.bind(this), this.closeConnectionTask.bind(this)], function (err, result) {
      // TODO: Consider calling this.closeConnectionTask() here to ensure it's closed even if an error occurs in the series
      // TODO: Handle the case where err isn't null
      done();
    });
  ...
  compressReleaseTask(done) {
    ...
    let archiver = this.createArchiver(this.options.archiveType, this.options.archiveName, this.options.localPath, this.options.exclude);
    archiver.compress(fileSize => {
      spinner.stop();
  ...
  uploadArchiveTask(done) {
    ...
    this.remote.upload(this.options.archiveName, this.release.path, error => {
      spinner.stop();
  ...
  uploadReleaseTask(done) {
    ...
    this.remote.synchronize(this.options.localPath, this.release.path, this.options.deployPath + '/' + this.options.synchronizedFolder, () => {
      spinner.stop();
      this.logger.ok('Done');
      done();
    });
  ...
  updateSharedSymbolicLinkOnRemoteTask(done) {
    ...
    async.eachSeries(Object.keys(this.options.share), (currentSharedFolder, itemDone) => {
      const configValue = this.options.share[currentSharedFolder];
      let symlinkName = configValue;
      let mode = null;

      if (typeof configValue == 'object' && 'symlink' in configValue) {
  ...
  createFolderTask(done) {
    ...
    async.eachSeries(this.options.create, (currentFolderToCreate, itemDone) => {
  ...
  makeDirectoriesWritableTask(done) {
    ...
    async.eachSeries(this.options.makeWritable, (currentFolderToMakeWritable, itemDone) => {
  ...
  makeFilesExecutableTask(done) {
    ...
    async.eachSeries(this.options.makeExecutable, (currentFileToMakeExecutable, itemDone) => {
  ...
  remoteCleanupTask(done) {
    ...
    const folder = path.posix.join(this.options.deployPath, this.options.releasesFolder);
    this.remote.removeOldFolders(folder, this.options.releasesToKeep, () => {
      spinner.stop();
      this.logger.ok('Done');
      done();
    });
  ...
  middlewareCallbackExecute(commandsFunction, callback) {
    ...
    let commands = commandsFunction; // If commandsFunction is a function, take its result as commands
  ...
  populatePenultimateReleaseNameTask(done) {
    ...
    this.remote.getPenultimateRelease().then(penultimateReleasePath => {
  ...
  renamePenultimateReleaseTask(done) {
    ...
    this.remote.exec(['mv', path.posix.join(releasesPath, this.penultimateReleaseName), path.posix.join(releasesPath, newPreviousReleaseName)].join(' '), (err, exitCode, exitSignal, stdout, stderr) => {
      if (err) {
        done(err);
      }
